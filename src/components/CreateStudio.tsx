...
import { AlertTriangle, Check, Download, Palette, RotateCcw, Save, Sparkles } from 'lucide-react'
import type { CodeRecord } from '../types'
import { classifyContent } from '../utils/contentClassifier'

interface Props { add: (data: Omit<CodeRecord, 'id' | 'timestamp'>) => CodeRecord; notify: (message: string) => void }
...
const presets = [{ name: 'Mint / Ink', foreground: '#10251e', background: '#f5f3ed', shape: 'rounded' as QrShape }, { name: 'Night / Lime', foreground: '#d9ffe9', background: '#13231d', shape: 'dots' as QrShape }, { name: 'Cobalt / Snow', foreground: '#123d7a', background: '#f6f8ff', shape: 'classy-rounded' as QrShape }]

export default function CreateStudio({ add, notify }: Props) {
  ...
  const save = () => {
    if (mode === 'barcode' && barcodeError) { notify(barcodeError); return }
    const content = mode === 'qr' ? payload.trim() : barValue.trim()
    if (!content) { notify('Escribe un contenido antes de guardar'); return }
    add({ operation: 'generated', codeFormat: mode === 'qr' ? 'QR_CODE' : barFormat, content, contentType: classifyContent(content).type }); notify('Código guardado en historial')
  }
  const download = () => {
...
function ColorInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <label className="color-input"><span>{label}</span><input type="color" value={value} onChange={(event) => onChange(event.target.value)} /><code>{value.toUpperCase()}</code></label> }
